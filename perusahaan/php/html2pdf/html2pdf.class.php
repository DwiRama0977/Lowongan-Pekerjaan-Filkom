...
' . $content . '
		...
		protected function o_DIV($param, $other = 'div')
		{
			...
			if (!$this->sub_part && !$this->isSubPart) {
				switch ($this->style->value['text-align']) {
					case 'right':
						$x_corr = ($this->style->value['width'] - $w_reel);
						break;
					case 'center':
						$x_corr = ($this->style->value['width'] - $w_reel) * 0.5;
						break;
				}
				if ($x_corr > 0) $x_corr = 0;
				switch ($this->style->value['vertical-align']) {
					case 'bottom':
						$y_corr = ($this->style->value['height'] - $h_reel);
						break;
					case 'middle':
						$y_corr = ($this->style->value['height'] - $h_reel) * 0.5;
						break;
				}
			}
		...
		protected function o_BARCODE($param)
		{
			...
			$y = $this->pdf->getY();
			$w = $this->style->value['width'];
			if (!$w) $w = $this->style->ConvertToMM('50mm');
			$h = $this->style->value['height'];
			if (!$h) $h = $this->style->ConvertToMM('10mm');
		...
		protected function o_QRCODE($param)
		{
			...
			$h = $this->style->value['height'];
			$size = max($w, $h);
			if (!$size) $size = $this->style->ConvertToMM('50mm');
...
*/
		...
		protected function drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
		{
			...
			if (is_array($in_TL)) {
				$in_TL[0] -= $border['l']['width'];
				$in_TL[1] -= $border['t']['width'];
			}
			if (is_array($in_TR)) {
				$in_TR[0] -= $border['r']['width'];
				$in_TR[1] -= $border['t']['width'];
			}
			if (is_array($in_BR)) {
				$in_BR[0] -= $border['r']['width'];
				$in_BR[1] -= $border['b']['width'];
			}
			if (is_array($in_BL)) {
				$in_BL[0] -= $border['l']['width'];
				$in_BL[1] -= $border['b']['width'];
			}
		...
		protected function o_TD($param, $other = 'td')
		{
			...
			if ($this->sub_part) {
				...
				for ($j = 0; $j < $rowspan; $j++) {
					for ($i = 0; $i < $colspan; $i++) {
						HTML2PDF::$TABLES[$param['num']]['corr'][HTML2PDF::$TABLES[$param['num']]['corr_y'] + $j][HTML2PDF::$TABLES[$param['num']]['corr_x'] + $i] = ($i + $j > 0) ? '' : array($x, $y, $colspan, $rowspan);
					}
				}
			...
			for ($k = 0; $k < count($match[0]); $k++) {
				...
				switch ($nom) {
					...
					case 'translate':
						...
						else $val[0] = $this->style->ConvertToMM($val[0], $this->isInDraw['w']);
						if (!isset($val[1])) $val[1] = 0.;
						else $val[1] = $this->style->ConvertToMM($val[1], $this->isInDraw['h']);
					...
					case 'rotate':
						...
						if (!isset($val[1])) $val[1] = 0.;
						else $val[1] = $this->style->ConvertToMM($val[1], $this->isInDraw['w']);
						if (!isset($val[2])) $val[2] = 0.;
						else $val[2] = $this->style->ConvertToMM($val[2], $this->isInDraw['h']);
					...
					case 'matrix':
						...
						else $val[4] = $this->style->ConvertToMM($val[4], $this->isInDraw['w']);
						if (!isset($val[5])) $val[5] = 0.;
						else $val[5] = $this->style->ConvertToMM($val[5], $this->isInDraw['h']);
						$actions[] = $val;
						break;
				}
			}
